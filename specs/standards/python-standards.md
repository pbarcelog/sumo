...
- Tests: follow `specs/test-strategy.md` — pytest under `tests/tools/import/gis/**`; resolve binaries via `checkBinary`. For local smoke without compiling SUMO, `pip install eclipse-sumo` supplies `netconvert`/`sumo` on `PATH`.
