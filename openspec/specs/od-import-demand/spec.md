...
- **THEN** the OMX `NO` set equals `ZONE.NO` (726 ids), every **external-demand** zone resolves at
  least one `tazSource` and one `tazSink`, the 28 PuT-only zones are excluded, and `od2trips` emits a
  non-empty `trips.xml`
